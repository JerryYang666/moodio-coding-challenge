...
export function LazyVideo({
  ...
  const { isVisible, isNearViewport, isFarFromViewport } =
  ...
  // Safari does not reliably GC <video> elements' media resources (codec,
  // decode pipeline, network buffer) when they are removed from the DOM.
  // Without explicit cleanup, each navigation cycle (filter change, enter /
  // exit detail view) leaks resources, causing progressive scroll jank.
  useEffect(() => {
    return () => {
      ...
      video.load(); // Forces the browser to release the media pipeline
    };
  }, []);
      ...
      >
        ...
        <img
          ...
          className={`w-full h-full object-cover cursor-pointer transition-opacity duration-300 ${
            isLoaded ? "opacity-100 hover:opacity-80" : "opacity-0"
          }`}
        />
      </div>
    );
