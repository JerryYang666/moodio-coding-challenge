...
const VideoVisibilityContext =
  createContext<VideoVisibilityContextValue | null>(null);
...
export function VideoVisibilityProvider({
  ...
  const subscribersRef = useRef<
    Map<HTMLVideoElement, Set<(state: VisibilityState) => void>>
  >(new Map());

  // Track intersection state from each observer separately
  const nearIntersectionRef = useRef<
    Map<HTMLVideoElement, { isVisible: boolean; isNear: boolean }>
  >(new Map());
  ...
  const updateElementState = useCallback((element: HTMLVideoElement) => {
    const nearState = nearIntersectionRef.current.get(element) ?? {
      isVisible: false,
      isNear: false,
    };
  ...
  const register = useCallback(
    (element: HTMLVideoElement) => {
      ...
      const isNear =
        rect.bottom >= -nearMargin && rect.top <= viewportHeight + nearMargin;
      const isVisible = rect.bottom >= 0 && rect.top <= viewportHeight;
      const isWithinFarZone =
        rect.bottom >= -farMargin && rect.top <= viewportHeight + farMargin;
  ...
  const getState = useCallback((element: HTMLVideoElement): VisibilityState => {
    return (
      stateMapRef.current.get(element) ?? {
        isVisible: false,
  ...
  const contextValue = useMemo<VideoVisibilityContextValue>(
    () => ({ register, unregister, getState, subscribe }),
    [register, unregister, getState, subscribe],
  );
...
export function useVideoVisibility(
  ...
  if (!context) {
    throw new Error(
      "useVideoVisibility must be used within a VideoVisibilityProvider",
    );
  }
